...
export const Calendario: React.FC<Props> = () => {
  ...
  return (
    <div className="flex items-center justify-center mb-4 from-red-100 via-red-300 to-red-500 ">
      <div className="w-[550px] p-6 mx-auto rounded-2xl shadow-xl flex flex-col ">
        ...
        <div className="flex justify-between font-medium uppercase text-xs pt-4 pb-2 border-t">
          {["Dom", "Lun", "Mar", "Mie", "Jue", "Vie", "Sab"].map(
            (diaSemana) => (
              <div
                key={diaSemana}
                className="px-3 border rounded-sm w-14 h-5 flex items-center justify-center border-mlt-600 text-white bg-mlt-600 shadow-md"
              >
                {diaSemana}
              </div>
            )
        ...
        {[...Array(Math.ceil((diasEnMes + primerDiaDelMes) / 7))].map(
          (_, semanaIndex) => (
            <div
              key={semanaIndex}
              className="flex justify-between font-medium text-sm pb-2 select-none"
            >
              {[...Array(7)].map((_, diaIndex) => {
                ...
                const estiloDia = `px-1 w-14 flex justify-center items-center border hover:border-mlt-600 cursor-pointer ${
                  isSelected
